...
class PortfolioOptimizer:
    ...
    def __init__(self, risk_free_rate: float = 0.04):
        ...
        # Data fetcher via yfinance (inline)
    ...
    def build_portfolio(self, tickers: List[str], 
        ...
        hedge_fund = InvestmentCommittee()  # Use rules for speed
        analyses = []
        
        for ticker in tickers:
            try:
                result = hedge_fund.analyze(ticker, auto_fetch_data=True)
                ...
                emoji = {"bullish": "📈", "bearish": "📉", "neutral": "➡️"}[str(result.consensus.value)]
                print(f"{emoji} {ticker:6} {str(result.consensus.value).upper():8} ({result.average_confidence:2}%)", file=sys.stderr)
        ...
        qualified = [a for a in analyses if str(a["result"].consensus.value) != "bearish"]
        if len(qualified) < 2:
        ...
        for i, a in enumerate(qualified):
            if weights[i] >= min_position:
                assets.append(PortfolioAsset(
                    ticker=a["ticker"],
                    weight=weights[i],
                    signal=str(a["result"].consensus.value),
                    confidence=a["result"].average_confidence,
    ...
    def _estimate_return(self, analysis, historical: Optional[Dict]) -> float:
        """Estimate expected annual return based on analysis and historical data"""
        
        # Base return estimate from signal
        if str(analysis.consensus.value) == "bullish":
            base_return = 0.12 + (analysis.average_confidence / 100) * 0.08  # 12-20%
        elif str(analysis.consensus.value) == "neutral":
            base_return = 0.06 + (analysis.average_confidence / 100) * 0.04  # 6-10%
        else:  # bearish
            base_return = -0.05  # -5%
    ...
    def analyze_existing_portfolio(self, holdings: Dict[str, float]) -> PortfolioAnalysis:
        ...
        hedge_fund = InvestmentCommittee()
        assets = []
        ...
        for ticker, weight in holdings.items():
            try:
                ...
                assets.append(PortfolioAsset(
                    ticker=ticker,
                    weight=weight,
                    signal=str(result.consensus.value),
                ...
                emoji = {"bullish": "📈", "bearish": "📉", "neutral": "➡️"}[str(result.consensus.value)]
                print(f"{emoji} {ticker:6} {weight:5.1%} {str(result.consensus.value).upper():8}", file=sys.stderr)
            except Exception as e:
